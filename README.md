...
자연어 회의실 예약
"오늘 4시에 보이저 예약해줘" → 자동 예약 (비밀번호 미입력 시 1234 기본 설정)

시간 및 상대 날짜 자동 보정
"내일 2시" → 찜꽁 운영시간 기준 14:00 으로 인식

층별 빈 회의실 조회
"지금 12층에 빈 회의실 있어?" → 현재 예약 가능한 공간 안내

조건부 가용 회의실 자동 배정
"페어룸 빼고 아무데나 예약해줘" → 조건에 맞는 빈 방 탐색 후 예약

유연한 예약 시간 설정
"오늘 2시에 30분 예약해줘" → 기본 1시간 외 맞춤 시간 지원
...
Spring Boot App
├─ Slack Event Handler
├─ Claude API Client       ← 자연어 → 예약 파라미터 파싱
└─ 찜꽁 API Client         ← 실제 예약 / 조회 요청
│
▼
찜꽁 Backend API
...
찜꽁 백엔드 API를 MCP(Model Context Protocol)로 감싸 Claude와 연결하는 구조로,
예약 로직을 재구현하지 않고 실제 서비스와 동일한 신뢰도로 동작합니다.
